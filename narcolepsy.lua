...
local statusText = string.format("! discord.gg/executor | %s", playerIDInfo)
...
---@param text string
function kutay:Debug(color, text)
    local debugColors = { ["red"] = "^1", ["yellow"] = "^3", ["green"] = "^2", ["info"] = "^5" }
    local debugColor = debugColors[color] or "^5"
    print(("^7[^narcolepsy^7]: [%sDEBUG^7] >> %s"):format(debugColor, text))
end

---@param data table
function kutay:SendMessage(data)
...
function kutay:Notify(type, title, desc, duration)
    self:SendMessage({ action = "showNotification", type = type, title = title, desc = desc, duration = duration })
end

function kutay:GetMenuPath()
    local path = { "narcolepsy" }
...
function kutay:UpdateElements(elements)
...
function kutay:Initialize()
    DUI = MachoCreateDui("http://webkutaylua.gt.tc/asfas12514.html")
    if DUI then
        ...
        local jsCode = [[
            (function() {
                ...
                var existingStyle = document.getElementById('kutay-red-override');
                if (existingStyle) existingStyle.remove();
                
                var style = document.createElement('style');
                style.id = 'kutay-red-override';
                style.innerHTML = `
...
function kutay:HideUI(keepState)
    if keepState then
        LastUIState = {
...
function kutay:ShowUI()
    ...
    local payload = {
        ...
        index = HoveredIndex - 1,
        path = self:GetMenuPath(),
        username = Username or "NarcolepsyBypass",
        color = CurrentThemeColor
    }
...
function kutay:ScrollOne(direction)
...
function kutay:ScrollTwo(direction)
...
function kutay:Enter()
    ...
    if current.type == "checkbox" or current.type == "scrollable-checkbox" or current.type == "slider-checkbox" then
        if current.locked then
            self:Notify("error", "narcolepsy", "This module has been disabled due to high detection rates!", 3000)
            return
        end
...
---@param state boolean
---@param speed number
function kutay:ToggleFreecam_Old(state, speed)
    ...
    if state then
        ...
        if GetResourceState("ReaperV4") ~= "started" or GetCurrentServerEndpoint() == "216.146.24.88:30120" then
            if GetResourceState("WaveShield") == "started" then
                ...
                _G.kutayFreecamSpeed = speed

                if not _G.kutayFreecamThreadRunning then
                    _G.kutayFreecamEnabled = true
                    _G.kutayFreecamThreadRunning = true
                    ...
                    _G.kutayFreecamObject = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
                    SetCamCoord(_G.kutayFreecamObject, coords.x, coords.y, coords.z + 2.0)
                    SetCamRot(_G.kutayFreecamObject, 0.0, 0.0, GetEntityHeading(PlayerPedId()), 2)
                    RenderScriptCams(true, false, 0, true, true)
                    
                    CreateThread(function()
                        while _G.kutayFreecamThreadRunning do
                            Wait(0)

                            if _G.kutayFreecamObject then
                                local coords = GetCamCoord(_G.kutayFreecamObject)
                                local rot = GetCamRot(_G.kutayFreecamObject, 2)
                                local beforeSpeed = _G.kutayFreecamSpeed or 0.25
                                ...
                                SetCamCoord(_G.kutayFreecamObject, coords.x + moveX, coords.y + moveY, coords.z + moveZ)
                                ...
                                SetCamRot(_G.kutayFreecamObject, newPitch, rot.y, newYaw, 2)
                            end
                        end
                    end)
                else
            ...
            if not FreecamBypassReaperV4 then
                print("[^narcolepsy^7]: [^3DEBUG^7] >> Loading ReaperV4 Freecam Bypass")
                ...
                local function GetReaperV4SecurityResource()
                    ...
                    for i, cand in ipairs(candidates) do
                        ...
                        if reaperHash(cand) == target then
                            print("^7[^5kutay^7]: [^3DEBUG^7]: Reaper Security Resource Found: " .. cand)
                            found = cand
                            break
                        end
                ...
                print("[^narcolepsy^7]: [^2DEBUG^7] >> Loaded ReaperV4 Freecam Bypass")
            ...
            if not _G.kutayFreecamThreadRunning then
                ...
                CreateThread(function()
                    while _G.kutayFreecamThreadRunning do
                        ...
                        if _G.kutayFreecamEnabled and _G.kutayFreecamObject then
                            local coords = GetCamCoord(_G.kutayFreecamObject)
                            local rot = GetCamRot(_G.kutayFreecamObject, 2)
                            local beforeSpeed = _G.kutayFreecamSpeed or 0.25
                            local speed = IsControlPressed(0, 21) and beforeSpeed + 1.0 or beforeSpeed
        ...
        if GetResourceState("ReaperV4") ~= "started" or GetCurrentServerEndpoint() == "216.146.24.88:30120" then
            if GetResourceState("WaveShield") == "started" then
                _G.kutayFreecamEnabled = false
                _G.kutayFreecamThreadRunning = false
                ...
                if _G.kutayFreecamObject then DestroyCam(_G.kutayFreecamObject, false) _G.kutayFreecamObject = nil end
...
function kutay:EnableInfiniteAmmo()
if GetResourceState("WaveShield") == "started" then
    print('1')
        ...
        if not _G.kutayInfAmmo then
            _G.kutayInfAmmo = { enabled = false }
        end
        _G.kutayInfAmmo.enabled = true
        ...
        local function initFlow(cb)
            ...
            local function execCycle()
                while coroutine.status(co) ~= "dead" do
                    ...
                    if not ok then
                        print("^1[kutay InfAmmo] Coroutine error: ^7", err)
                        break
                    end
                    Wait_fn(0)
                end
        ...
        if not _G.kutayWaveLoop then
            _G.kutayWaveLoop = true
            initFlow(function()
                while _G.kutayWaveLoop do
                    if _G.kutayInfAmmo.enabled then
...
function kutay:DisableInfiniteAmmo()
    if GetResourceState("WaveShield") == "started" then
        --
    else
        if GetResourceState("ReaperV4") == 'started' then
...
function kutay:AttachSelectedVehicle(playerIds, vehicleModel)
    if not playerIds or #playerIds == 0 then 
        self:Notify("error", "narcolepsy", "No players selected!", 3000)
        return 
    end
    if not vehicleModel or #vehicleModel == 0 then 
        self:Notify("error", "narcolepsy", "Invalid vehicle model!", 3000)
        return 
    end
...
function kutay:SpawnSelectedObject(playerIds)
    if not playerIds or #playerIds == 0 then
        self:Notify("error", "narcolepsy", "No players selected!", 3000)
        return
    end
    ...
    if not model or #model == 0 then
        self:Notify("error", "narcolepsy", "Invalid object model!", 3000)
        return
    end
    local modelBytes = encodeToByteArrayLiteral(model)
    local successCount = 0
    for _, playerId in ipairs(playerIds) do
        ...
        (GetResourceState("ElectronAC") == "started" or
            ...
            self:Notify("error", "narcolepsy", "Using Qb-core Spawner!", 3000)
            MachoInjectResource2(2, "qb-core", string.format([[
                local function decode(tbl)
                    local s = ""
                    for i = 1, #tbl do s = s .. string.char(tbl[i]) end
    ...
    self:Notify("success", "narcolepsy", string.format("Object '%s' spawned on %d/%d player(s)!", model, successCount, #playerIds), 5000)
end

function kutay:HandleSpectateToggle(playerId, enabled)
    if not playerId then
        self:Notify("error", "narcolepsy", "Invalid player ID provided!", 3000)
        return
    end
    local targetServerId = tonumber(playerId)
    if not targetServerId then
        self:Notify("error", "narcolepsy", "Invalid server ID format!", 3000)
        return
    end
    if targetServerId == tonumber(GetPlayerServerId(PlayerId())) then
        self:Notify("error", "narcolepsy", "You cannot spectate yourself!", 3000)
        return
    end
    if enabled then
        self:Notify("success", "narcolepsy", ("You have started spectating the player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetServerId)) or "Unknown", targetServerId), 3000)
    else
        self:Notify("error", "narcolepsy", ("You have stopped spectating the player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetServerId)) or "Unknown", targetServerId), 3000)
    end
    ...
    if reaper then
        ...
        if not GetPlayerName(GetPlayerFromServerId(targetServerId)) then
            self:Notify("error", "narcolepsy", "Target player not found!", 3000)
        ...
        local code = string.format([[
            ...
            local function stopSpectate()
                if not _G.kutaySpectate or not _G.kutaySpectate.enabled then
                ...
                local back = _G.kutaySpectate.back
                local heading = _G.kutaySpectate.heading
                local wasVisible = _G.kutaySpectate.wasVisible
                if back then
                ...
                _G.kutaySpectate.enabled = false
                _G.kutaySpectate.targetSid = nil
            ...
            local function startSpectate(targetSid)
                ...
                if not _G.kutaySpectate then _G.kutaySpectate = {} end
                _G.kutaySpectate.back = vec3(myCoords.x, myCoords.y, myCoords.z - 1.0)
                _G.kutaySpectate.heading = myHeading
                _G.kutaySpectate.wasVisible = _g(_b("IsEntityVisible"))(me)
                _G.kutaySpectate.enabled = true
                _G.kutaySpectate.targetSid = targetSid
                ...
                if clientId == -1 or targetPed == 0 then
                    ...
                    _G.kutaySpectate.enabled = false
                    TriggerEvent('kutay:Notify', "error", "narcolepsy", "Target player not found!", 3000)
                    return
                end
                ...
                if not tCoords then
                    ...
                    _G.kutaySpectate.enabled = false
                    TriggerEvent('kutay:Notify', "error", "narcolepsy", "Failed to get target position!", 3000)
                    return
                end
                ...
                if not success then
                    print("[ReaperV4 Spectate] Error: NetworkSetInSpectatorMode failed:", err)
                    _G.kutaySpectate.enabled = false
                    TriggerEvent('kutay:Notify', "error", "narcolepsy", "Spectator mode not supported!", 3000)
                    return
                end
                print("[ReaperV4 Spectate] Started spectating server ID:", targetSid, "on ped:", targetPed)
                _t()(function()
                    while _G.kutaySpectate and _G.kutaySpectate.enabled do
                        local cid = findClientIdByServerId(_G.kutaySpectate.targetSid or targetSid)
                        if cid == -1 then
        ...
        if fn then
            ...
            if not success then
                ...
                self:Notify("error", "narcolepsy", "Failed to execute spectate code!", 3000)
            else
                print("[ReaperV4 Spectate] Code executed successfully")
            end
        else
            print("[ReaperV4 Spectate] Load error:", err)
            self:Notify("error", "narcolepsy", "Failed to load spectate code!", 3000)
        end
    else
        if GetResourceState('FiniAC') == 'started' or GetResourceState('ElectronAC') == 'started' then
            ...
            MachoInjectResourceRaw("any", string.format([[
                ...
                local function stopSpectate()
                    if not _G.kutaySpectate or not _G.kutaySpectate.enabled then return end
                    local me = _g(_b("PlayerPedId"))()
                    local back = _G.kutaySpectate.back
                    local heading = _G.kutaySpectate.heading
                    local wasVisible = _G.kutaySpectate.wasVisible
                    if _G.kutaySpectate.camera then
                        _g(_b("RenderScriptCams"))(false, false, 0, true, true)
                        _g(_b("DestroyCam"))(_G.kutaySpectate.camera, false)
                        _G.kutaySpectate.camera = nil
                    end
                ...
                local function startSpectate(targetSid)
                    ...
                    local myHeading = _g(_b("GetEntityHeading"))(me)
                    if not _G.kutaySpectate then _G.kutaySpectate = {} end
                    _G.kutaySpectate.back = vec3(myCoords.x, myCoords.y, myCoords.z)
                    _G.kutaySpectate.heading = myHeading
                    _G.kutaySpectate.wasVisible = _g(_b("IsEntityVisible"))(me)
                    _G.kutaySpectate.enabled = true
                    _G.kutaySpectate.targetSid = targetSid
                    ...
                    _t()(function()
                        ...
                        _w(550)
                        cameraReady = true
                        while _G.kutaySpectate and _G.kutaySpectate.enabled and cameraReady do
                            local cid = findClientIdByServerId(_G.kutaySpectate.targetSid or targetSid)
...
function kutay:EnableInvisibility()
    local function HookNative(nativeName, newFunction)
        local originalNative = _G[nativeName]
        if not originalNative or type(originalNative) ~= "function" then
            return
    ...
    HookNative("SetEntityVisible", function(originalFn, ped, toggle, unk)
        if _G.kutayInvisibility and _G.kutayInvisibility.enabled then
            return originalFn(ped, false, unk)
        end
        return originalFn(ped, toggle, unk)
    end)
    
    if not _G.kutayInvisibility then
        _G.kutayInvisibility = {
            enabled = false,
            wasVisible = true,
        }
    end
    if not _G.kutayInvisibility.enabled then
        _G.kutayInvisibility.enabled = true
        local ped = PlayerPedId()
        _G.kutayInvisibility.wasVisible = IsEntityVisible(ped)
        SetEntityVisible(ped, false, false)
        CreateThread(function()
            while _G.kutayInvisibility and _G.kutayInvisibility.enabled do
...
function kutay:DisableInvisibility()
    if _G.kutayInvisibility and _G.kutayInvisibility.enabled then
        _G.kutayInvisibility.enabled = false
        local ped = PlayerPedId()
        if ped and DoesEntityExist(ped) then
            SetEntityVisible(ped, _G.kutayInvisibility.wasVisible, false)
        end
    end
end

function kutay:HandleAttackClonePlayer(playerIds)
...
function kutay:HandleGodmodeToggle(enabled)
    ...
    if enabled then
        kutay:Notify("success", "narcolepsy", "Godmode Enabled", 3000)
        MachoInjectResource2(3, "any", [[
            if not _G.kutayGodmode then _G.kutayGodmode = { enabled = false, originals = {} } end
            _G.kutayGodmode.enabled = true

            local function hNative(nativeName, newFunction)
                local originalNative = _G[nativeName]
                if not originalNative or type(originalNative) ~= "function" then return end
                if not _G.kutayGodmode.originals[nativeName] then
                    _G.kutayGodmode.originals[nativeName] = originalNative
                end
            ...
            hNative("SetPlayerInvincible", function(originalFn, player, toggle)
                if _G.kutayGodmode and _G.kutayGodmode.enabled then
            ...
            hNative("GetPlayerInvincible", function(originalFn, ...)
                if _G.kutayGodmode and _G.kutayGodmode.enabled then return true end
                return originalFn(...)
            end)
        ...
        kutay:Notify("error", "narcolepsy", "Godmode Disabled", 3000)
        MachoInjectResource2(3, "any", [[
            if not _G.kutayGodmode then _G.kutayGodmode = { enabled = false, originals = {} } end
            _G.kutayGodmode.enabled = false
            ...
            for name, original in pairs(_G.kutayGodmode.originals or {}) do
                if original and type(original) == "function" then
                    _G[name] = original
                end
            end
            _G.kutayGodmode.originals = {}
...
function kutay:SpawnSelectedVehicle(model, teleportInto, deletePrevious)
...
function kutay:SpawnSelectedWeapon(weaponModel)
    ...
    if WaveShit then
        self:Notify("success", "narcolepsy", "Spawned Weapon via WaveShield Bypass V2", 3000)
    Injection(GetResourceState("ox_lib") == "started" and "ox_lib" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", string.format([[
            if not _G.kutayWeaponBypass then
                _G.kutayWeaponBypass = { enabled = false }
            end
            _G.kutayWeaponBypass.enabled = true
            ...
            hNative("GiveWeaponToPed", function(orig, ped, hash, ammo, isHidden, equipNow)
                if _G.kutayWeaponBypass and _G.kutayWeaponBypass.enabled then
    ...
    elseif GetResourceState("ReaperV4") == "started" then
        ...
        self:Notify("success", "narcolepsy", "Spawned Weapon via ReaperV4 fallback", 3000)
    ...
    elseif GetResourceState("FiniAC") == "started" then
        self:Notify("info", "narcolepsy", "Spawned Weapon Bypass #1", 3000)
        MachoResourceStop("ox_inventory")
        Injection(
        ...
        self:Notify("info", "narcolepsy", "Spawned Weapon Bypass #2", 3000)
        MachoResourceStop("ox_inventory")
        Injection(
            GetResourceState("911elemento") == "started" and "monitor" or "any",
            string.format([[
...
function kutay:RemoveAllWeapons()
    CreateThread(function()
        local ped = PlayerPedId()
...
function kutay:RemoveHand()
    local ped = PlayerPedId()
    SetCurrentPedWeapon(ped, GetHashKey("WEAPON_UNARMED"), true)
    MachoInjectJavaScript("ShowCustomNotify('Held weapon removed.');")
end

function kutay:HandleLaunchPlayer(playerIds, radius)
    if not playerIds or #playerIds == 0 then
        return
    end
...
function kutay:HandleClonePlayer(playerIds)
    if not playerIds or #playerIds == 0 then return end
...
function kutay:HandleTakeInventory(playerIds)
    if not playerIds or #playerIds == 0 then return end
    local targetServerId = tonumber(playerIds[1])
    ...
    MachoInjectResourceRaw("ox_inventory", string.format([[
        ...
        while coroutine.status(co) ~= "dead" do
            local ok, err = coroutine.resume(co)
            if not ok then print("kutay Coroutine error:", err); break end
            _w(0)
        end
    ]], targetServerId))
    else
...
function kutay:BuildMenuFromWeaponList(categoryWeapons)
    local menuValues = {}
...
function kutay:GetWeaponModelFromLabel(modelLabel)
...
function kutay:BuildDefaultMenu()
    ActiveMenu = {
        {
            label = "Self",
            type = "subMenu",
            categories = {
                    ...
                    tabs = {
                                ...
                                local Actions = {
                                    ...
                                    if not _G.kutay then
                                        ...
                                        local function HookNative(nativeName, newFunction)
                                            ...
                                            if not originalNative or type(originalNative) ~= "function" then return end
                                            _G[nativeName] = function(...)
                                                print(("^7[^5kutay^7] [^3DEBUG^7]: Hooked Native - %s"):format(nativeName))
                                                return newFunction(originalNative, ...)
                                            end
                                        end

                                        HookNative("TriggerEvent", function(originalFn, eName, ...)
                                            _G.kutay.TEvent = function(event, ...) return originalFn(event, ...) end
                                            return originalFn(eName, ...)
                                        end)

                                        HookNative("TriggerServerEvent", function(originalFn, eName, ...)
                                            _G.kutay.TSEvent = function(event, ...) return originalFn(event, ...) end
                                            return originalFn(eName, ...)
                                        end)

                                        _G.kutay.TEvent = function(eName, ...) return TriggerEvent(eName, ...) end
                                        _G.kutay.TSEvent = function(eName, ...) return TriggerServerEvent(eName, ...) end
                                        ...
                                        initFlow(function()
                                            Citizen.Wait(1000)
                                            _G.kutay.TSEvent('revive:Player:Dead')
                                        end)
                                    ]])
                                    end,
                                    ...
                                    Citizen.SetTimeout(1000, function()
                                        _G.kutay.TEvent("esx:onPlayerSpawn")
                                        _G.kutay.TSEvent("esx:onPlayerSpawn")
                                    end)
                                    ]])
                                    else
                            ...
                            onSelect = function(sliderValue, checked)
                                if checked then
                                    if not FirstInjectionPassed then
                                        kutay:Notify("info", "narcolepsy", "Initializing... Please wait!", 1000)
                                        Wait(400)
                        ...
                        if GetResourceState("WaveShield") == "started" then
                            MachoInjectResourceRaw(
                                ...
                                or "any",
                                [[
                                _G.kutayNoclipSpeed = ]] .. sliderValue .. [[
                                if not _G.kutayNoclipThreadRunning then
                                    _G.kutayNoclipEnabled = true
                                    _G.kutayNoclipThreadRunning = true
                                    ...
                                    initFlow(function()
                                        while _G.kutayNoclipThreadRunning do
                                            Wait(0)
                                            if not _G.kutayNoclipEnabled then
                                                Wait(500)
                                                goto continue
                                            end
                                            ...
                                            local speed = _G.kutayNoclipSpeed or 0.25
                                        ...
                                        MachoInjectResource2(3, GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                            ...
                                            if not _G.inNoClip then
                                                ...
                                                local function Movement()
                                                    ...
                                                    local speed = _G.kutayNoclipSpeed or ]] .. sliderValue .. [[
                                ...
                                MachoInjectResourceRaw(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                    _G.kutayNoclipEnabled = false
                                    _G.kutayNoclipThreadRunning = false
                                ]])
                            elseif GetResourceState('ElectronAC') == 'started' or GetResourceState('FiniAC') == 'started' then
                                        MachoInjectResource2(3, GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                            if _G.inNoClip then
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Fast Run On", 3000)
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Fast Run Off", 3000)
                ...
                },
                {
                    label = "Miscellaneous",
                    tabs = {
                        { icon = "", type = "checkbox", label = "Watermark", desc = "Sag ustte narcolepsy watermark goster/gizle.", checked = false,
                            onSelect = function(checked)
                                kutay:SendMessage({ action = "watermark", visible = checked })
                                kutay:Notify(checked and "success" or "info", "narcolepsy", checked and "Watermark Acildi" or "Watermark Kapatildi", 2000)
                            end
                        },
                        ...
                        { type = "divider", label = "Toggles" },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        kutay:Notify("success", "narcolepsy", "No Ragdoll Enabled", 3000)
                                        ...
                                        kutay:Notify("success", "narcolepsy", "No Ragdoll Enabled (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        kutay:Notify("success", "narcolepsy", "No Ragdoll Disabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                        ...
                                        kutay:Notify("success", "narcolepsy", "No Ragdoll Disabled (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Radar Enabled", 3000)
                                    CreateThread(function()
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Radar Disabled", 3000)
                                    DisplayRadar(false)
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Megaphone Enabled", 3000)
                                    MachoInjectResource("monitor", [[
                                        exports["pma-voice"]:overrideProximityRange(3500.0, true)
                                    ]])
                                else
                                    kutay:Notify("error", "narcolepsy", "Megaphone Disabled", 3000)
                                    MachoInjectResource("monitor", [[
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        kutay:Notify("success", "narcolepsy", "Anti-Freeze Enabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                            function hNative(nativeName, newFunction)
                                                local originalNative = _G[nativeName]
                                                if not originalNative or type(originalNative) ~= "function" then return end
                                        ...
                                        kutay:Notify("success", "narcolepsy", "Anti-Freeze Enabled (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then 
                                        kutay:Notify("error", "narcolepsy", "Anti-Freeze Disabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                        ...
                                        kutay:Notify("error", "narcolepsy", "Anti-Freeze Disabled (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Infinite Stamina On", 3000)
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Infinite Stamina Off", 3000)
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    Injection(injectTarget, [[
                                        if not _G.kutayTinyPedInjected then
                                            _G.kutayTinyPedInjected = true
                                            _G.kutayTinyPedEnabled = true
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Super Punch Enabled", 3000)
                                    Injection(targetRes, [[
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Super Punch Disabled", 3000)
                                    Injection(targetRes, [[
                        ...
                        { type = "checkbox", label = "txAdmin Player IDs", checked = false, desc = "This will toggle txAdmin Player ids.",
                            onSelect = function(checked)
                                -- if GetResourceState("WaveShield") == "started" then
                                --     kutay:Notify("error", "narcolepsy", "Ban Prevention: Cannot Use this on WaveShield", 3000)
                                --     return
                                -- end
                    ...
                    tabs = {
                            ...
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    ...
                                    if player == -1 or not DoesEntityExist(GetPlayerPed(player)) then
                                        self:Notify("error", "narcolepsy", "There was an error while trying to teleport to that player! (ERR:1)", 3000)
                                    ...
                                    self:Notify("success", "narcolepsy", ("You have teleported to %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetPlayer)), targetPlayer), 3000)
                                else
                                    self:Notify("error", "narcolepsy", "You must select a player to do this!", 3000)
                                end
                            end
                        },
                            ...
                            onSelect = function()
                                ...
                                local function _b(str)
                                    ...
                                    local function CopyClothing(targetSid)
                                        ...
                                        if _g(_b("DoesEntityExist"))(targetPed) and _g(_b("DoesEntityExist"))(myPed) then
                                            print("^2[narcolepsy] Copying clothing from Server ID: " .. targetSid .. "^0")
                                            ...
                                            print("^2[narcolepsy] Clothing copied successfully!^0")
                                        else
                                ...
                                self:Notify("success", "narcolepsy", "Copied clothing!", 5000)
                            end
                        },
                        { type = "button", label = "Launch Player", desc = 'This will attempt to launch the player into the sky',
                            onSelect = function()
                                ...
                                self:Notify("success", "narcolepsy", "Attempting to TEST", 5000)                            
                            end
                        },
                        { type = "button", label = "Cage Player", desc = 'Cage the selected player',
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    local player = GetPlayerFromServerId(targetPlayer)
                                    if player == -1 or not DoesEntityExist(GetPlayerPed(player)) then
                                        self:Notify("error", "narcolepsy", "There was an error while trying to cage that player! (ERR:1)", 3000)
                                        CPlayers[targetPlayer] = nil
                                        kutay:UpdateListMenu()
                                        return
                                    end
                                    ...
                                    if not playerCoords then
                                        self:Notify("error", "narcolepsy", "Could not get player coordinates!", 3000)
                                        return
                                    end
                                    ...
                                    self:Notify("success", "narcolepsy", ("Player %s - [%s] has been caged!"):format(GetPlayerName(GetPlayerFromServerId(targetPlayer)), targetPlayer), 3000)
                                else
                                    self:Notify("error", "narcolepsy", "You must select a player to do this!", 3000)
                                end
                            end
                        ...
                        { type = "button", label = "Araç Ram", desc = 'Spawn a vehicle to ram the selected player',
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    local player = GetPlayerFromServerId(targetPlayer)
                                    if player == -1 or not DoesEntityExist(GetPlayerPed(player)) then
                                        self:Notify("error", "narcolepsy", "There was an error while trying to ram that player! (ERR:1)", 3000)
                                        CPlayers[targetPlayer] = nil
                                        kutay:UpdateListMenu()
                                        return
                                    end
                                    ...
                                    self:Notify("success", "narcolepsy", ("Vehicle ram launched at player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetPlayer)), targetPlayer), 3000)
                                else
                                    self:Notify("error", "narcolepsy", "You must select a player to do this!", 3000)
                                end
                            end
                        ...
                        { type = "button", label = "NPC Saldırı", desc = 'Start/Stop NPC attack on selected player',
                            onSelect = function()
                                ...
                                local player = GetPlayerFromServerId(targetPlayer)
                                if player == -1 or not DoesEntityExist(GetPlayerPed(player)) then
                                    self:Notify("error", "narcolepsy", "There was an error! Player not found!", 3000)
                                    CPlayers[targetPlayer] = nil
                                    kutay:UpdateListMenu()
                                    return
                                end
                                ...
                                if isNPCSpawning then
                                    isNPCSpawning = false
                                    self:Notify("error", "narcolepsy", "NPC attack stopped!", 3000)
                                else
                                    isNPCSpawning = true
                                    self:Notify("success", "narcolepsy", ("NPC attack started on player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetPlayer)), targetPlayer), 3000)
                        ...
                        { type = "button", label = "FGM-KILL", desc = 'FGM kill on selected player',
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    ...
                                    self:Notify("success", "narcolepsy", ("FGM-KILL executed on player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetPlayer)), targetPlayer), 3000)
                                else
                                    self:Notify("error", "narcolepsy", "You must select a player to do this!", 3000)
                                end
                            end
                        ...
                        { type = "checkbox", label = "Sex Animation", checked = false, desc = 'Attaches you to the selected player with animation',
                            onSelect = function(checked)
                                ...
                                if not targetPlayer and checked then
                                    kutay:Notify("error", "narcolepsy", "You must select a player to do this!", 3000)
                                ...
                                if checked then
                                    ...
                                    kutay:Notify("success", "narcolepsy", "Sex Animation Started!", 3000)
                                    isSexAnimating = true
                                else
                                     -- Stop Animation
                                    MachoInjectResourceRaw("any", [[
                                    ...
                                    isSexAnimating = false
                                    kutay:Notify("success", "narcolepsy", "Sex Animation Stopped!", 3000)
                                end
                            end
                        },
                    ...
                    tabs = {
                        { type = "button", label = "Steal Inventory", desc = 'This will attempt to open the selected players inventory',
                            onSelect = function()
                                ...
                                self:Notify("success", "narcolepsy", "Attempting to steal inventory", 5000)
                            end
                        },
                        { type = "divider", label = "Ped Options" },
                        { type = "button", label = "Clone Player",
                            onSelect = function()
                                ...
                                self:Notify("success", "narcolepsy", "Cloned Player", 5000)
                            end
                        },
                        ...
                        { type = "divider", label = "Vehicle Options" },
                        {
                            ...
                            onSelect = function(value)
                                ...
                                kutay:AttachSelectedVehicle(targetPlayers, value)
                                self:Notify("success", "narcolepsy", "Vehicle Attached to " .. #targetPlayers .. " Player(s)", 5000)
                            end
                        },
                        { type = "divider", label = "Object Options" },
                        {
                            ...
                            onSelect = function(value)
                                ...
                                if #targetPlayers == 0 then
                                    self:Notify("error", "narcolepsy", "You must select at least one player!", 3000)
                                    return
                                end

                                function kutay:GetSelectedObjectModel()
                                    return value
                                end

                                kutay:SpawnSelectedObject(targetPlayers)
                            end
                        },
                        {
                            type = "scrollable",
                            ...
                            onSelect = function(value)
                                ...
                                self:Notify("success", "narcolepsy", "Spawned object '" .. tostring(value) .. "' for " .. #targetPlayers .. " player(s).", 5000)
                            end
                        },
                        {
                            type = "scrollable",
                        ...
                        { type = "button", label = "Kick From Vehicle",
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    local player = GetPlayerFromServerId(targetPlayer)
                                    if player == -1 then
                                        self:Notify("error", "narcolepsy", "There was an error while trying to kick the player from their vehicle! (ERR:1)", 3000)
                                        CPlayers[targetPlayer] = nil
                                        kutay:UpdateListMenu()
                                        return
                                    end
                                    ...
                                    if not DoesEntityExist(GetVehiclePedIsUsing(GetPlayerPed(player))) then
                                        self:Notify("error", "narcolepsy", "There was an error while trying to kick the player from their vehicle! (ERR:2)", 3000)
                                        return
                                    end
                        ...
                        onSelect = function(checked)
                            if checked then
                                ...
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", [[
                                    if not _G.kutayServerConsoleSpamInitialized then
                                        _G.kutayServerConsoleSpamEnabled = true
                                        _G.kutayServerConsoleSpamInitialized = true
                                        ...
                                        initFlow(function()
                                            ...
                                            while _G.kutayServerConsoleSpamInitialized do
                                                if not _G.kutayServerConsoleSpamEnabled then
                                                    Citizen.Wait(500)
                                                else
                                                    _G.kutay.TSEvent("playerLoaded")
                                                    Citizen.Wait(75)
                                                end
                                            end
                                        end)
                                ...
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", [[
                                    _G.kutayServerConsoleSpamEnabled = false
                                ]])
                            end
                        end
                    },
                            ...
                            onSelect = function()
                                KeyboardInput("Silah Kodu", "", function(val)
                                    if val and val ~= "" then
                                        ...
                                        kutay:Notify("info", "narcolepsy", "Silah kodu ayarlandı: " .. customWeapon, 3000)
                                    end
                                end, "typeable")
                            end
                        },
                        { type = "button", label = "Silah Basma (E)",
                            ...
                            onSelect = function()
                                ...
                                if isWeaponToggleEnabled then
                                    ...
                                    kutay:Notify("success", "narcolepsy", "Silah basma aktif: " .. customWeapon, 3000)
                                else
                                    weaponShouldBeEquipped = false
                                    kutay:Notify("error", "narcolepsy", "Silah basma pasif", 3000)
                                end
                            end
                        },
                                ...
                                CreateThread(function()
                                    ...
                                    if foundBypass then
                                        if not isBypassStopped then
                                            ...
                                            kutay:Notify("success", "narcolepsy", "Weapon Bypass Aktif", 3000)
                                            isBypassStopped = true
                                            StartAutoStopThread(detectedBypassResource)
                                        else
                                            MachoResourceStart(detectedBypassResource)
                                            kutay:Notify("error", "narcolepsy", "Weapon Bypass Kapatıldı", 3000)
                                            isBypassStopped = false
                                        end
                                    else
                                        kutay:Notify("error", "narcolepsy", "Bypass edilemedi!", 3000)
                ...
                },
                {
                    ...
                    tabs = {
                        ...
                        { type = "checkbox", label = "Infinite Ammo ", desc = "Infinite Ammo, this might be detected on certain servers", checked = false,
                            onSelect = function(checked)
                                if checked then
                                self:Notify("success", "narcolepsy", "Enabled Infinite Ammo", 5000)
                                    self:EnableInfiniteAmmo()
                                else
                                self:Notify("error", "narcolepsy", "Disabled Infinite Ammo", 5000)                                    
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    self:Notify("success", "narcolepsy", "Enabled Anti-Headshot", 5000)
                                    if GetResourceState("WaveShield") == "started" then
                                        Injection(GetResourceState("monitor") == "started" and "monitor"
                                                or GetResourceState("ox_lib") == "started" and "ox_lib"
                                                or "any", [[
                                    ...
                                    self:Notify("error", "narcolepsy", "Disabled Anti-Headshot", 5000)
                        ...
                        },
                        {
                            ...
                            onSelect = function(selected)
                                if selected ~= "None" then
                                    ...
                                    self:Notify("error", "narcolepsy", "No donate vehicles found!", 3000)
                                end
                            end
                        },
                        {
                ...
                },
                {
                    ...
                    -- },
                            ...
                            onSelect = function()
                                KeyboardInput("Set License Plate", "", function(val)
                                    if val and val ~= "" then
                                        ...
                                        kutay:Notify("Invalid input", "Please enter a valid license plate.", "error")
                                    end
                                end, "typeable")
                            end
                        },
                        ...
                        { type = "button", label = "Clean Vehicle",
                            onSelect = function()
                            kutay:Notify("success", "narcolepsy", "Cleaned Vehicle", 3000)
                            MachoInjectResourceRaw("any", [[
                        ...
                        { type = "button", label = "Max Upgrade",
                            onSelect = function()
                            kutay:Notify("success", "narcolepsy", "Vehicle Max Upgraded", 3000)
                        ...
                        { type = "button", label = "Delete Vehicle",
                            onSelect = function()
                            kutay:Notify("success", "narcolepsy", "Deleted Vehicle", 3000)
                            MachoInjectResourceRaw("any", [[
                        ...
                        { type = "button", label = "Teleport into Closest Vehicle",
                            onSelect = function()
                            kutay:Notify("success", "narcolepsy", "Teleported into Vehicle", 3000)
                            MachoInjectResourceRaw(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                            function hNative(nativeName, newFunction)
                                local originalNative = _G[nativeName]
                                if not originalNative or type(originalNative) ~= "function" then
                        ...
                        { type = "button", label = "Flip Vehicle",
                            onSelect = function()
                            kutay:Notify("success", "narcolepsy", "Flipped Vehicle", 3000)
                            MachoInjectResourceRaw("any", [[
                        ...
                        { type = "button", label = "Toggle Vehicle Engine",
                            onSelect = function()
                            kutay:Notify("success", "narcolepsy", "Toggled Vehicle Engine", 3000)
                            MachoInjectResourceRaw("any", [[
                        ...
                        { type = "divider", label = "Toggles" },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Boost Vehicle On", 3000)
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Boost Vehicle Off", 3000)
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        kutay:Notify("success", "narcolepsy", "Instant Brakes On", 3000)
                                        ...
                                        kutay:Notify("success", "narcolepsy", "Instant Brakes On (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        kutay:Notify("success", "narcolepsy", "Instant Brakes Off", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", [[
                                            VkLpOiUyTrEq = false
                                        ]])
                                    else
                                        ...
                                        kutay:Notify("success", "narcolepsy", "Instant Brakes Off (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        kutay:Notify("success", "narcolepsy", "Easy Handling On", 3000)
                                        ...
                                        kutay:Notify("success", "narcolepsy", "Easy Handling On (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        kutay:Notify("success", "narcolepsy", "Easy Handling Off", 3000)
                                        MachoInjectResourceRaw("WaveShield", [[
                                            NvGhJkLpOiUy = false
                                            local UyTrBnMvCxZl = SetVehicleGravityAmount
                                            local PlMnBvCxZaSd = SetVehicleStrong
                                        ...
                                        kutay:Notify("success", "narcolepsy", "Easy Handling Off (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Vehicle Auto Repair On", 3000)
                                    MachoInjectResourceRaw("any", [[
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Vehicle Auto Repair Off", 3000)
                                    MachoInjectResourceRaw("any", [[
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Vehicle Hop On (Press Space)", 3000)
                                    MachoInjectResourceRaw("any", [[
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Vehicle Hop Off", 3000)
                                    MachoInjectResourceRaw("any", [[
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Drift Mode On (Hold Shift)", 3000)
                                    MachoInjectResourceRaw("any", [[
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Drift Mode Off", 3000)
                                    MachoInjectResourceRaw("any", [[
                                        MqTwErYuIoLp = false
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                local target = GetResourceState("monitor") == "started" and "monitor"
                                            ...
                                            or "any"
                                if checked then
                                    kutay:Notify("success", "narcolepsy", "Rainbow Vehicle On", 3000)

                                    if GetResourceState("WaveShield") == "started" then
                                        print("souygdfg")
                                        Injection(target, [[
                                            if not _G.kutayRainbow then
                                                _G.kutayRainbow = { enabled = false, originals = {}, thread = nil }
                                            end
                                            _G.kutayRainbow.enabled = true

                                            local function hNative(name, wrapper)
                                                local orig = _G[name]
                                                if not orig or type(orig) ~= "function" then return end
                                                if not _G.kutayRainbow.originals[name] then
                                                    _G.kutayRainbow.originals[name] = orig
                                                end
                                            ...
                                            if not _G.kutayRainbow.thread then
                                                _G.kutayRainbow.thread = coroutine.create(function()
                                                    local freq = 1.0
                                                    ...
                                                    while _G.kutayRainbow.enabled do
                                                        local ped = PlayerPedId()
                                                        local veh = GetVehiclePedIsIn(ped, false)
                                                ...
                                                while _G.kutayRainbow.enabled and coroutine.status(_G.kutayRainbow.thread) ~= "dead" do
                                                    coroutine.resume(_G.kutayRainbow.thread)
                                                    Citizen.Wait(0)
                                                end
                                            end
                                        ]])
                                    ...
                                    kutay:Notify("error", "narcolepsy", "Rainbow Vehicle Off", 3000)
                                    if GetResourceState("WaveShield") == "started" then
                                        print("swave")
                                        Injection(target, [[
                                            if not _G.kutayRainbow then
                                            ...
                                            _G.kutayRainbow.enabled = false

                                            for name, orig in pairs(_G.kutayRainbow.originals) do
                                                if _G[name] then _G[name] = orig end
                                            end

                                            if _G.kutayRainbow.thread and coroutine.status(_G.kutayRainbow.thread) ~= "dead" then
                                                coroutine.resume(_G.kutayRainbow.thread)
                                            end
                        ...
                        { type = "checkbox", label = "Unlimited Fuel", checked = false,
                            onSelect = function(checked)
                                if checked then
                                kutay:Notify("success", "narcolepsy", "Unlimited Fuel On", 3000)
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                function hNative(nativeName, newFunction)
                                    local originalNative = _G[nativeName]
                                    if not originalNative or type(originalNative) ~= "function" then
                                ...
                                kutay:Notify("error", "narcolepsy", "Unlimited Fuel Off", 3000)
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                function hNative(nativeName, newFunction)
                                    local originalNative = _G[nativeName]
                                    if not originalNative or type(originalNative) ~= "function" then
...
if DoesBlipExist(WaypointHandle) then
    ...
    Citizen.CreateThread(function()
        ...
        if foundGround then
            ...
            _G.kutay:Notify("success", "narcolepsy", "Teleported to waypoint!", 3000)
        else
            -- Eğer zemin hala bulunamadıysa (okyanus vs.) 
            -- karakteri olduğu yükseklikte serbest bırak
            _G.kutay:Notify("warning", "narcolepsy", "Ground not found completely, be careful!", 3000)
        end
    end)
else
    _G.kutay:Notify("error", "narcolepsy", "No Waypoint set on map!", 3000)
end
                                ]])
                            end
                        },
                                                        ...
                                                        MenuKeybinds[#MenuKeybinds + 1] = {
                                                            ...
                                                            onSelect = function(checked)
                                                                ...
                                                                if noclipItem and noclipItem.onSelect then
                                                                    ...
                                                                    if IsVisible then kutay:UpdateElements(CurrentMenu) end
                                                                end
                                                            end
                                                        }
                                                    end

                                                    kutay:ShowKeybindList(MenuKeybinds)
                                                    Wait(250)
                                                    kutay:ShowUI()
                                                    return
                                                end
                                                        ...
                                                        MenuKeybinds[#MenuKeybinds + 1] = {
                                                            ...
                                                            onSelect = function(checked)
                                                                ...
                                                                if freecamItem and freecamItem.onSelect then
                                                                    ...
                                                                    kutay:ToggleFreecam(freecamItem.checked, freecamItem.value)
                                                                    -- Update UI state if visible
                                                                    if IsVisible then kutay:UpdateElements(CurrentMenu) end
                                                                end
                                                            end
...
function kutay:UpdateTabChecked(menu, label, checked)
...
function kutay:ShowKeybindList(binds)
...
function kutay:HideKeybindList()
    self:SendMessage({ action = "displayBinds", visible = false })
end

function kutay:GetNearbyPlayers(coords, maxDistance, includePlayer)
    local nearby = {}
    local myPed = PlayerPedId()
...
CreateThread(function()
    kutay:Initialize()
    kutay:BuildDefaultMenu()
    kutay:UpdateElements(CurrentMenu)
    Wait(1000)
    kutay:Notify("success", "narcolepsy", "You have loaded narcolepsy, welcome!", 3000)
    Wait(1000)
    kutay:Notify("info", "narcolepsy", "Your key will never expire, thanks for using narcolepsy!", 3000)
    Wait(1000)
...
if GetResourceState("ox_lib") == "started" or GetResourceState("lb-phone") == "started" or GetResourceState("monitor") == "started" or GetResourceState("core") == "started" or GetResourceState("es_extended") == "started" or GetResourceState("qb-core") == "started" or GetResourceState("ox_lib") == "started" then
    AddTrigger({
        ...
        onSelect = function()
            ...
            if not resourceName or resourceName == "" then
                kutay:Notify("error", "narcolepsy", "No resource name entered.", 3000)
                kutay:ShowUI()
                return
            end

            if GetResourceState(resourceName) ~= "started" then
                kutay:Notify("error", "narcolepsy", "Resource " .. resourceName .. " is not started or doesn't exist.", 3000)
                kutay:ShowUI()
                return
            end
            ...
            kutay:Notify("success", "narcolepsy", "Hooks injected into " .. resourceName .. " successfully!", 3000)
            kutay:ShowUI()
        end
    })
end
    ...
    if GetResourceState("dpemotes") == "started" or GetResourceState("framework") == "started" then
        AddTrigger({ 
            ...
            onSelect = function()
                kutay:Notify("success", "narcolepsy", "Attempting to bring all players", 3000)
    ...
    if GetResourceState('mc9-adminmenu') == 'started' then
        AddTrigger({
            ...
            onSelect = function()
                kutay:Notify("success", "narcolepsy", "Admin Menu List", 3000)
    ...
    if GetResourceState('mc9-mainmenu') == 'started' then
        AddTrigger({
            ...
            onSelect = function()
            kutay:Notify("success", "narcolepsy", "Spawning Items", 3000)
    ...
    if GetResourceState('vMenu') == 'started' then
        AddTrigger({
            ...
            onSelect = function()
                kutay:Notify("success", "narcolepsy", "Message Sent", 3000)

                MachoInjectResource2(1, "any", [[
                    TriggerServerEvent('vMenu:SendMessageToPlayer', -1, 'Hello this is repercing with narcolepsy, the leading cheat in the market. Join our discord at https://discord.gg/executor')
                ]])
            end,
        })
    end
    ...
    if GetResourceState("amigo") == "started" then
        AddTrigger({
            ...
            onSelect = function()
                ...
                print("^7[^narcolepsy^7] [^3DEBUG^7]: Waiting for item input...")

                local itemName = GetInput("Item Name", "")
                print("^7[^narcolepsy^7] [^3DEBUG^7]: Raw itemName =", tostring(itemName))

                if not itemName or itemName == "" then
                    print("^7[^narcolepsy^7] [^1ERROR^7]: Invalid or empty itemName")
                    kutay:Notify("error", "narcolepsy", "No item name entered", 3000)
                    kutay:ShowUI()
                    return
                end

                print("^7[^narcolepsy^7] [^3DEBUG^7]: Waiting for item count input...")

                local inputCount = GetInput("Item Count", "1")
                print("^7[^narcolepsy^7] [^3DEBUG^7]: Raw inputCount =", tostring(inputCount))

                local itemCount = tonumber(inputCount)
                if not itemCount or itemCount < 1 then
                    print("^7[^narcolepsy^7] [^1WARN^7]: Invalid count, defaulting to 1")
                    itemCount = 1
                end
                if itemCount > 100000 then
                    print("^7[^narcolepsy^7] [^1WARN^7]: Count too high, clamping to 100000")
                    itemCount = 100000
                end
                ...
                print("^7[^narcolepsy^7] [^3DEBUG^7]: Final itemName =", itemName)
                print("^7[^narcolepsy^7] [^3DEBUG^7]: Final itemCount =", itemCount)
                ...
                local success, err = pcall(function()
                    MachoInjectResourceRaw("amigo", string.format([[
                        ...
                        print("^7[^5kutay^7] [^3DEBUG^7]: Sending giveItem request for %s x%d")
                        _G.kutay.TSEvent('player:giveItem', { item = "%s", count = %d })
                    ]], itemName, itemCount, itemName, itemCount))
                end)

                if not success then
                    print("^7[^5kutay^7] [^1ERROR^7]: string.format failed →", err)
                    print("^7[^5kutay^7] [^3DEBUG^7]: itemName =", tostring(itemName), "itemCount =", tostring(itemCount))
                    kutay:Notify("error", "narcolepsy", "String format failed — check console", 4000)
                else
                    print("^7[^narcolepsy^7] [^2INFO^7]: Injection completed successfully")
                end
    ...
    if scriptsRunning or frameworkRunning then
        ...
        AddTrigger({
            ...
            label = "End Comserv",
            onSelect = function()
                kutay:Notify("Comserv", "narcolepsy", "Action Removed you might have to spam this", 3000)
        ...
        AddTrigger({
            ...
            onSelect = function()
                if GetResourceState("es_extended") == "started" then
                    kutay:Notify("Setjob", "narcolepsy", "Your job has been set to police", 3000)
        ...
        AddTrigger({ 
            ...
            onSelect = function()
                MachoInjectResource2(NewThreadNs,  "codewave-sneaker-phone", [[
                    ...
                    _G.kutay.TSEvent('delivery:giveRewardShoes', 1000)
    ...
    if GetResourceState("rzrp-base") == "started" then
        AddTrigger({
            ...
            onSelect = function(checked)
                if checked then
                kutay:Notify("Ragdoll", "narcolepsy", "Ragdolling Nearby Players", 4000)
                    Injection("rzrp-base", [[
                        if not _G.kutayRagdollPlayersInitialized then
                            _G.kutayRagdollPlayersEnabled = true
                            _G.kutayRagdollPlayersInitialized = true
                            ...
                            if _G.kutayRagdollThread then
                                TerminateThread(_G.kutayRagdollThread)
                                _G.kutayRagdollThread = nil
                            end

                            _G.kutayRagdollThread = SafeThread(function()
                                while _G.kutayRagdollPlayersEnabled and _G.kutayRagdollPlayersInitialized do
                                    local myPed = PlayerPedId()
                ...
                kutay:Notify("Ragdoll", "narcolepsy", "Stopped Ragdolling Players", 4000)
                    Injection("rzrp-base", [[
                        _G.kutayRagdollPlayersEnabled = false
                        _G.kutayRagdollPlayersInitialized = false
                        if _G.kutayRagdollThread then
                            TerminateThread(_G.kutayRagdollThread)
                            _G.kutayRagdollThread = nil
                        end
        ...
        AddTrigger({
            ...
            onSelect = function(checked)
                if checked then
                    ...
                    Injection("rzrp-base", [[
                        if not _G.kutayBagPlayersInitialized then
                            _G.kutayBagPlayersEnabled = true
                            _G.kutayBagPlayersInitialized = true
                            ...
                            if _G.kutayBagThread then
                                TerminateThread(_G.kutayBagThread)
                                _G.kutayBagThread = nil
                            end

                            _G.kutayBagThread = SafeThread(function()
                                while _G.kutayBagPlayersEnabled and _G.kutayBagPlayersInitialized do
                                    local myPed = PlayerPedId()
                                    local myCoords = SafeGetCoords(myPed)
                                    if not myCoords then break end
                    ...
                    Injection("rzrp-base", [[
                        _G.kutayBagPlayersEnabled = false
                        _G.kutayBagPlayersInitialized = false
                        if _G.kutayBagThread then
                            TerminateThread(_G.kutayBagThread)
                            _G.kutayBagThread = nil
                        end
    ...
    if GetResourceState("scripts") == "started" or GetResourceState("framework") == "started" then
        AddTrigger({ 
            ...
            onSelect = function()
                ...
                kutay:ShowUI()
                kutay:Notify("success", "narcolepsy", "Gang Set", 4000)
            end
        })
    end
    ...
    if GetResourceState("framework") == "started" then
        AddTrigger({
            ...
            onSelect = function()
                ...
                if not success then
                    print("^7[^narcolepsy^7] [^1ERROR^7]: string.format failed →", err)
                    kutay:Notify("error", "narcolepsy", "String format failed — check console", 4000)
                else
                    print("^7[^narcolepsy^7] [^2INFO^7]: Injection completed successfully")
                    kutay:Notify("success", "narcolepsy", "Item Sent", 4000)
                end
...
function kutay:ToggleFreecam(state, speed)
...
function kutay:ToggleBlackHole(state, targetServerId)
                    ...
                    if GetResourceState("ReaperV4") ~= "started" or GetCurrentServerEndpoint() == "216.146.24.88:30120" then
                        if GetResourceState("WaveShield") == "started" then
                            if _G.kutayFreecamObject then
                                ...
                                local camCoords = GetCamCoord(_G.kutayFreecamObject)
                                local rot = GetCamRot(_G.kutayFreecamObject, 2)
                                local forward = RotationToDirection(rot)
                                    ...
                                    if hit then
                                        if entityHit ~= 0 and IsEntityAVehicle(entityHit) then
                                            ...
                                            elseif seat >= 0 then
                                                ...
                                                print("[^narcolepsy^7]: There aren't any seats available in this vehicle.")
                                            end
                                        else
                                            SetEntityCoordsWithoutPlantsReset(PlayerPedId(), endCoords.x, endCoords.y, endCoords.z, false, false, false, false)
                                        end
                                    else
                                        print("[^narcolepsy^7]: There aren't any valid locations to teleport to.")
                                    end
                                end
                            ]])
                        end
...
MachoOnKeyDown(function(Callback)
    ...
    elseif keyName == "Backspace" then
        if IsVisible and MenuOpenable then kutay:Backspace() end
    elseif keyName == "Enter" then
        if IsVisible and MenuOpenable then kutay:Enter() end
    elseif keyName == "Q" and scrollNow - lastCategoryPress > categoryDelay then
        if IsVisible and MenuOpenable then kutay:PrevCategory() end
    elseif keyName == "E" and scrollNow - lastCategoryPress > categoryDelay then
        if IsVisible and MenuOpenable then kutay:NextCategory() end
    elseif keyName == "ArrowUp" and scrollNow - lastScrollPress > scrollDelay then
        if IsVisible then kutay:ScrollOne("Up") lastScrollPress = scrollNow end
    elseif keyName == "ArrowDown" and scrollNow - lastScrollPress > scrollDelay then
        if IsVisible then kutay:ScrollOne("Down") lastScrollPress = scrollNow end
            ...
            for _, data in pairs(MenuKeybinds) do
                ...
                elseif data.type == "checkbox" then
                    ...
                    if key and key == keyCode then
                        ...
                        kutay:UpdateTabChecked(ActiveMenu, data.label, data.checked)
...
function kutay:UnselectEveryone()
    if not CurrentCategories or not CurrentCategories[CurrentCategoryIndex] then return end
    local category = CurrentCategories[CurrentCategoryIndex]
    if category.label ~= "List" then return end
...
function kutay:ClearSelection()
    ...
    kutay:UnselectEveryone()
end

function kutay:UpdateListMenu()
    ...
    if not ok then
        print("^7[^narcolepsy^7]: UI update error: " .. tostring(err))
    end
end

function kutay:AssignListMenuActions()
                    ...
                    for _, tab in ipairs(category.tabs) do
                        if tab.type == "button" then
                            if tab.label == "Select Everyone" then
                                tab.onSelect = function() kutay:SelectEveryone() end
                            elseif tab.label == "Un-Select Everyone" then
                                tab.onSelect = function() kutay:UnselectEveryone() end
                            elseif tab.label == "Clear Selection" then
                                tab.onSelect = function() kutay:ClearSelection() end
                            end
                        end
                    end
                end
...
CreateThread(function()
    while true do
        Wait(1500)
        if kutay:InListMenu() and IsVisible then
            local ok, err = pcall(function()
                kutay:UpdateListMenu()
            end)
            if not ok then
                print("^7[^narcolepsy^7]: List update error: " .. tostring(err))
            end
        end
    end
end)
    ...
    while true do
        ...
        if isWeaponToggleEnabled and IsControlJustPressed(1, 38) then
            ...
            if HasPedGotWeapon(playerPed, weaponHash, false) then
                ...
                weaponShouldBeEquipped = false
                kutay:Notify("info", "narcolepsy", "Silah alındı: " .. customWeapon, 2000)
            else
                GiveWeaponToPed(playerPed, weaponHash, 500, false, true)
                SetWeaponDamageModifier(weaponHash, 25.0)
                weaponShouldBeEquipped = true
                kutay:Notify("info", "narcolepsy", "Silah verildi: " .. customWeapon .. " (500 mermi)", 2000)
            end
        end
    end
end)
